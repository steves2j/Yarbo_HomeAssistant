class S2JYarboMapCard extends HTMLElement {
  ...
  _ingestEmbeddedEntry(entry) {
    ...
    this._updateGpsCalibration(this._entry);
  ...
  _renderMapContent(entry, location, heading) {
    ...
    const drawing = this._buildMapDrawing(siteMap, location, heading);
    if (!drawing) {
      return `
        ${meta}
        <div class="map-card">
  ...
  _buildMapDrawing(siteMap, location, heading) {
  ...
  _renderCalibrationWarning(entry) {
    if (this._gpsCalibration || this._entryIsDocked(entry)) {
      return "";
    }

    return '<div class="map-warning">Calibration Missing. Dock to calibrate</div>';
  }
  ...
  _entryIsDocked(entry) {
    const statusState = entry?.status_entity_id
      ? this._hass?.states?.[entry.status_entity_id]
      : null;
    const statusValue = Number(statusState?.attributes?.charging_status);
    if (Number.isFinite(statusValue)) {
      return statusValue !== 0;
    }

    const summaryValue = Number(entry?.summary?.charging_status);
  ...
  _updateGpsCalibration(entry) {
    if (!entry?.site_map?.reference || !this._entryIsDocked(entry)) {
      return;
    }

    const location = this._entryLocation(entry);
    const rawLocalPoint = this._localPointFromLocation(location, entry.site_map.reference);
    if (!rawLocalPoint) {
      return;
    }

    const chargingPoints = Array.isArray(entry.site_map.charging_points)
      ? entry.site_map.charging_points
      : [];
    ...
    for (const dock of chargingPoints) {
      if (!dock?.point) {
        continue;
      }
      const distance = Math.hypot(
        Number(dock.point.x) - Number(rawLocalPoint.x),
        Number(dock.point.y) - Number(rawLocalPoint.y),
      );
    ...
    if (!nearestDock?.point || nearestDistance > 0.8) {
      return;
    }

    const dockForward = this._dockForwardVector(nearestDock);
    if (!dockForward) {
      return;
    }

    const dockPerpendicular = {
      x: -dockForward.y,
      y: dockForward.x,
    };
    const delta = {
      x: Number(nearestDock.point.x) - Number(rawLocalPoint.x),
      y: Number(nearestDock.point.y) - Number(rawLocalPoint.y),
    };

    this._gpsCalibration = {
      longitudinal: delta.x * dockForward.x + delta.y * dockForward.y,
      lateral: delta.x * dockPerpendicular.x + delta.y * dockPerpendicular.y,
    };
  ...
  _persistCalibration() {
    ...
    this._writeStorageJson(this._storageKey("calibration"), this._gpsCalibration);
  }

  _loadPersistedCalibration() {
    const stored = this._readStorageJson(this._storageKey("calibration"));
    const longitudinal = Number(stored?.longitudinal);
    const lateral = Number(stored?.lateral);
