class YarboOverviewCard extends HTMLElement {
  ...
  _createEntryElement(entryId) {
    ...
    section.innerHTML = `
      ...
      ${this._cardMode === "overview" || this._cardMode === "control" ? `
      <div class="device-card control-card">
        <div class="device-header">
          ...
          <div class="device-header-indicators">
            ...
            <div class="device-signal unknown" role="img" aria-label="Signal strength unavailable" title="Signal strength unavailable">
              <ha-icon class="device-signal-icon" icon="mdi:wifi-strength-alert-outline" aria-hidden="true"></ha-icon>
              <span class="device-signal-value">--</span>
            </div>
      ...
      ` : ""}
      ${this._cardMode === "overview" || this._cardMode === "map" ? `
      <div class="device-card map-card-shell">
      ...
      ` : ""}
      ${this._cardMode === "overview" || this._cardMode === "advanced" ? `
      <details class="device-card advanced-panel">
  ...
  _updateEntryElement(section, entry) {
    ...
    const signalBadge = section.querySelector(".device-signal");
    const signalValue = section.querySelector(".device-signal-value");
    const signalIcon = section.querySelector(".device-signal-icon");
    ...
    const signalTone = !hasSignal
      ...
      : signal >= 70
        ...
        : signal >= 35
          ...
          : "poor";
    if (signalValue) {
      signalValue.textContent = hasSignal ? `${Math.round(signal)}%` : "--";
    }
    if (signalBadge) {
      signalBadge.classList.remove("good", "fair", "poor", "unknown");
      signalBadge.classList.add(signalTone);
      const signalLabel = hasSignal
        ? `Wi-Fi signal strength ${Math.round(signal)} percent`
        : "Wi-Fi signal strength unavailable";
      signalBadge.setAttribute("aria-label", signalLabel);
      signalBadge.title = signalLabel;
    }
    if (signalIcon) {
      const icon = !hasSignal
        ...
        : signal >= 70
          ...
          : signal >= 50
            ...
            : signal >= 25
              ? "mdi:wifi-strength-2"
              : "mdi:wifi-strength-1";
      signalIcon.setAttribute("icon", icon);
    }
